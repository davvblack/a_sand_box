...
class Defend(object):
    ...
    def __init__(self, botId, facingDirection = None, description = ''):
        ...
        """
        The desired facing direction (Vector2) of the bot. If this is None the bot remains facing its current facing direction.
        """
...
class Move(object):
    ...
    def __init__(self, botId, target, description = ''):
        assert isinstance(botId, str)
        assert isinstance(target, Vector2) or isinstance(target, list)
...
class Attack(object):
    """
    Commands a bot to attack a specified position.
    """
